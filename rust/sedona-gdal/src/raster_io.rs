    ...
    for tile_y in 0..y_tile_count {
        for tile_x in 0..x_tile_count {
            ...
            let tile_upperleft_x =
                origin_upperleft_x + (x_offset as f64) * scale_x + (y_offset as f64) * skew_x;
            let tile_upperleft_y =
                origin_upperleft_y + (x_offset as f64) * skew_y + (y_offset as f64) * scale_y;
...
fn write_raster(
    ...
    for i in 0..raster_struct_array.len() {
        ...
        for band_index in 0..band_count {
            ...
            // Note: Some drivers (e.g., Zarr) don't support nodata values
...
mod tests {
    ...
    fn test_read_write_raster(
        ...
        let expected_new_length = (tile_count.0 * tile_size.0 / new_tile_width)
            * (tile_count.1 * tile_size.1 / new_tile_height);
        // Validate the new tiling
        assert_eq!(expected_new_length, raster_retiled_array.len());
