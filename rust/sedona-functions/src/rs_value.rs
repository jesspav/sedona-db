...
fn rs_value_doc() -> Documentation {
    Documentation::builder(
        DOC_SECTION_OTHER,
        format!("Returns the value at the given point in the raster.",),
        format!("RS_Value (raster: Raster, colX: Integer, colY: Integer, band: Integer)"),
    )
    .with_argument("raster", "Raster: Input raster")
    .with_argument("x", "Integer: X coordinate")
    .with_argument("y", "Integer: Y coordinate")
    .with_argument("band_id", "Integer: Band number (1-based index)")
    .with_sql_example(format!("SELECT RS_Value(raster, x, y, band_id)",))
    .build()
}
